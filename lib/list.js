    ...
    this.selectList = new SelectListView({
      ...
      elementForItem: (item, options) => {
        ...
        typeBlock.appendChild(
          highlightMatches(item.type, matches.map((x) => x - total))
        );
        total += 2 + item.type.length;
        priBlock.appendChild(
          highlightMatches(item.key, matches.map((x) => x - total))
        );
        ...
        secBlock.appendChild(
          highlightMatches(item.description, matches.map((x) => x - total))
        );
      ...
      try {
        ...
        for (const entry of entries) {
          ...
          this.items.push({
            ...
            type: entry.type,
            text: "@" + entry.type + " #" + entry.key + " | " + description,
            fPath: fPath,
          });
